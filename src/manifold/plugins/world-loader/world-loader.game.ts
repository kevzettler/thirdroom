...
import { SpawnPoint } from "../../core/component/SpawnPoint";
import { addChild } from "../../core/component/transform";
...
import {
  ...
  Thread,
} from "../../core/module/module.common";
import { addPeerId, NetworkModule, removePeerId } from "../../core/network/network.game";
import {
  EnterWorldMessage,
  WorldLoadedMessage,
...
import { GLTFResource, loadDefaultGLTFScene, loadGLTF } from "../../core/gltf/gltf.game";
import {
  addPhysicsBody,
  addPhysicsCollider,
  PhysicsModule,
  registerCollisionHandler,
} from "../../core/physics/physics.game";
import { boundsCheckCollisionGroups } from "../../core/physics/CollisionGroups";
import { Player } from "../../core/player/Player";
import { enableActionMap } from "../../core/input/ActionMappingSystem";
import { InputModule } from "../../core/input/input.game";
import { spawnEntity } from "../../core/utils/spawnEntity";
import { addScriptComponent, loadScript, Script } from "../../core/scripting/scripting.game";
import { ColliderType, PhysicsBodyType, SamplerMapping } from "../../core/resource/schema";
import {
  ResourceModule,
  getRemoteResource,
...
import { findResourceRetainerRoots, findResourceRetainers } from "../../core/resource/findResourceRetainers";
import { RemoteResource } from "../../core/resource/RemoteResourceClass";
...
import {
  ...
  spawnPlayer,
} from "../../core/player/PlayerRig";
import { MAX_OBJECT_CAP } from "../../core/config.common";
...
export interface WorldLoaderModuleState {
...
export const WorldLoaderModule = defineModule<GameContext, WorldLoaderModuleState>({
  name: "world-loader",
  create() {
    return {
  ...
  async init(ctx) {
    const { worldLoaderMessages } = getModule(ctx, WorldLoaderModule);
...
function enterWorld(ctx: GameContext, localPeerId?: string) {
  ...
  const physics = getModule(ctx, PhysicsModule);
  const input = getModule(ctx, InputModule);
  const { environmentScript, environmentGLTFResource } = getModule(ctx, WorldLoaderModule);
