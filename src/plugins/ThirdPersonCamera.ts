...
type ThirdPersonCameraModuleState = {};

export const ThirdPersonCameraModule = defineModule<GameState, ThirdPersonCameraModuleState>({
  ...
  init(ctx) {
    ...
    enableActionMap(controller, ThirdPersonCameraActionMap);
...
export const ThirdPersonCameraActions = {
  Look: "ThirdPersonCamera/Look",
};

export const ThirdPersonCameraActionMap: ActionMap = {
  id: "third-person-camera",
  actions: [
    {
      id: "look",
      path: ThirdPersonCameraActions.Look,
...
export const ThirdPersonCameraPitchTarget = defineComponent({
  maxAngle: Types.f32,
  minAngle: Types.f32,
  sensitivity: Types.f32,
});
export const ThirdPersonCameraYawTarget = defineComponent({
  sensitivity: Types.f32,
});
...
export function addCameraPitchTargetComponent(world: World, eid: number) {
  addComponent(world, ThirdPersonCameraPitchTarget, eid);
  ThirdPersonCameraPitchTarget.maxAngle[eid] = 89;
  ThirdPersonCameraPitchTarget.minAngle[eid] = -89;
  ThirdPersonCameraPitchTarget.sensitivity[eid] = DEFAULT_SENSITIVITY;
}

export function addCameraYawTargetComponent(world: World, eid: number) {
  addComponent(world, ThirdPersonCameraYawTarget, eid);
  ThirdPersonCameraYawTarget.sensitivity[eid] = DEFAULT_SENSITIVITY;
}

export const cameraPitchTargetQuery = defineQuery([ThirdPersonCameraPitchTarget, Transform]);
export const cameraYawTargetQuery = defineQuery([ThirdPersonCameraYawTarget, Transform]);

function applyYaw(ctx: GameState, controller: InputController, eid: number) {
  const [lookX] = controller.actions.get(ThirdPersonCameraActions.Look) as vec2;

  if (Math.abs(lookX) >= 1) {
    const sensitivity = ThirdPersonCameraYawTarget.sensitivity[eid] || 1;
...
function applyPitch(ctx: GameState, controller: InputController, eid: number) {
  const [, lookY] = controller.actions.get(ThirdPersonCameraActions.Look) as vec2;

  if (Math.abs(lookY) >= 1) {
    const rotation = Transform.rotation[eid];
    const sensitivity = ThirdPersonCameraPitchTarget.sensitivity[eid] || DEFAULT_SENSITIVITY;
    const maxAngle = ThirdPersonCameraPitchTarget.maxAngle[eid];
    const minAngle = ThirdPersonCameraPitchTarget.minAngle[eid];
...
export function ThirdPersonCameraSystem(ctx: GameState) {
  const network = getModule(ctx, NetworkModule);
